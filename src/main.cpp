...
void initialize(void) {
  ...
  std::cout << "EC   : Move camera along y axis." << std::endl;
  std::cout << "QZ   : Change the camera movement speed." << std::endl;
...
void keyCallback(GLFWwindow *window, int key, int scancode, int action,
  ...
  if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
    ...
    flyscene->createDebugRay(mouse_pos, Eigen::Vector3f(0,0,0), Eigen::Vector3f(0, 0, 0), 0, 5);
